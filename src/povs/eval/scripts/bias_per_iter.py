...
params = BiasPerIterParams(
    ...
    deck_size=1024,
    max_iterations=6,
    ngrams_specs=[NgramSpec.parse(x) for x in [2, 3]],
...
result = bias_per_iteration(
    ...
    ngrams_specs=params.ngrams_specs,
...
report = BiasPerIterReport(
    ...
    biases=pd.DataFrame({
        ...
        **{spec.title: result.ngram_tvds[:, i] for i, spec in enumerate(params.ngrams_specs)},
    ...
    plot=plot_bias_per_iteration(
        ...
        ngram_names=[x.title for x in params.ngrams_specs],
