...
void povs_cuda(DType* Xg_ptr, long num_instances, const long* Oh_ptr, long num_offsets, int iterations, int seed, int8_t device_id);
...
#define INSTANTIATE_POVS_CUDA(DType, PBlockSize, VBlockSize, InstanceSize)                                                   \
    template void povs_cuda<DType, PBlockSize, VBlockSize, InstanceSize>(                                                    \
        DType * Xg_ptr, long num_instances, const long* Oh_ptr, long num_offsets, int iterations, int seed, int8_t device_id \
    );
